...
**Tests.** `cd agent && node --test`. **191 tests et 187 verts au 2026-09-08 après la phase 7**, dont 14 dans `queryBrain.test.js` et 22 dans `capture.test.js`, qui tournent sans réseau ni Notion ni Google ni classifier. **Quatre tests sont rouges depuis avant le découpage**, tous dans `dates.test.js` sur les fuseaux Toronto : c'est la référence, ni plus ni moins après un changement. `npm install` est nécessaire sur un clone frais, sinon trois fichiers échouent faute de `@anthropic-ai/sdk`.
